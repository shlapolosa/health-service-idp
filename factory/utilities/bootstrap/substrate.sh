...
# kept for reference; it still works against the new factory/substrate/ layout).
#
...
apply_dir       "$REPO_ROOT/factory/substrate/crossplane/rbac"           "--recursive"
apply_dir       "$REPO_ROOT/factory/substrate/crossplane/cluster-gateway" "--recursive"

info "Crossplane XRDs + Compositions (substrate-level)"
apply_dir       "$REPO_ROOT/factory/substrate/crossplane"
apply_dir       "$REPO_ROOT/factory/substrate/crossplane/compositions"
apply_dir_recursive "$REPO_ROOT/factory/substrate/crossplane/templates"

info "real-time platform manifests (PostgreSQL, MQTT, Kafka, Lenses, Metabase)"
apply_dir_recursive "$REPO_ROOT/factory/substrate/crossplane/realtime-platform"

info "GraphQL substrate"
apply_dir_recursive "$REPO_ROOT/factory/substrate/crossplane/graphql"

# ───── 2. Argo Workflows server config + RBAC ─────
info "Argo Workflows server config + RBAC"
apply_dir "$REPO_ROOT/factory/substrate/argo"

# ───── 3. Argo Events (OAM webhook + watcher) ─────
info "Argo Events sensors + OAM webhook"
apply_dir "$REPO_ROOT/factory/substrate/argo-events"

# ───── 4. ArgoCD ─────
info "ArgoCD vcluster app templates"
apply_dir "$REPO_ROOT/factory/substrate/argocd"

# ───── 5. Knative Serving ─────
info "Knative Serving install + autoscaler config"
apply_dir "$REPO_ROOT/factory/substrate/knative"
