...
**Component invariants — `analytics-platform`** (warehouse/OLAP overlay). When the user asks for analytics, a data warehouse, OLAP, or reporting over streaming/transactional data, reach for `analytics-platform` — a thin OVERLAY that adds a warehouse (Snowflake today) plus an inline roundtrip (ingest → Kafka Connect → Lenses SQL → warehouse → Lenses stream-republish). Rules: it REUSES an existing `realtime-platform` when one is present (set `realtime: <name>`); if none is declared, submit-time auto-injects one — do NOT add a second realtime-platform yourself. EXACTLY ONE `analytics-platform` per app. Pick `ingestion.mode: cdc` for Debezium Postgres CDC off a `postgresql`/`neon-postgres` component (set `ingestion.source` + `tables`), or `ingestion.mode: gateway` to consume topics already on the realtime-platform. The warehouse seam is backed by the `snowflake` CD (emits `<name>-conn` from Key Vault `kv-socrates-6706`); examples live in `catalog/examples/analytics-platform-example.yaml` (auto-inject) and `analytics-platform-reuse-realtime-example.yaml` (reuse).
