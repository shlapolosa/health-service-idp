"""Entra JWT validation middleware — same pattern as capability-mcp-server/src/interface/auth.py.

Mirror, not import, so the factory MCP has no source-coupling to the catalog MCP (they're separate
deployment units with different trust boundaries). Set AUTH_DISABLED=true for local dev.

The factory MCP enforces a stricter allow-list than the catalog: only `sp-architect-writer`'s OID
should reach this MCP in production. Configure via ALLOWED_OIDS.
"""
...
class EntraJWTMiddleware(BaseHTTPMiddleware):
    def __init__(self, app):
        ...
        self.audience = os.getenv("APIM_AUDIENCE", "")
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if self.allowed_oids and claims.get("oid") not in self.allowed_oids:
            return JSONResponse({"error": "caller not in factory allow-list"}, status_code=403)
        request.state.caller = claims.get("oid", "unknown")
        return await call_next(request)
