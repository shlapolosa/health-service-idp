"""Entra JWT validation middleware (mirror of catalog/factory pattern).

In production the WEB MCP sits behind APIM which validates the subscription key. This middleware
is defence-in-depth — if the MCP is ever exposed without APIM, it still rejects unauthenticated
callers. Set AUTH_DISABLED=true for local dev or when APIM is the only ingress.
"""
