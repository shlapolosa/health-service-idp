...
1. Merge CD + skeleton composition + XRD (this PR; additive, nothing references them yet).
2. Complete the composition skeleton: Postgres, Redis, `<name>-svix-credentials` init
   Job (mint JWT secret + admin token), event-type reconcile Job — copy verbatim from
   `realtime-platform-claim-composition.yaml` and retarget.
...
4. Apply CD/XRD/composition to the cluster (operator); deploy `webhookdemo` OAM.
