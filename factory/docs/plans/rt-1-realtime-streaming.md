...
## 13. IMPLEMENTED (2026-06-07)

All seven workstreams landed on branch `worktree-agent-af1c80ffead99f55d`. Validated: every edited YAML `yaml.safe_load`s; all CUE templates brace/bracket/paren-balanced and parse via `cue_param_parser`; capability-mcp-core pytest 74/74 green; generated realtime `src/main.py` python-compiles and its imports resolve to real agent-common modules.

### Done
1. **Declarative topics** — `realtime-platform.cd.yaml` gains `topics: [{name,partitions,retention}]` → flows into `RealtimePlatformClaim.spec.topics` (XRD extended) → surfaced as `TOPIC_<name>` keys on the `<name>-conn` secret. New composition resource `realtime-topic-provisioning-job` envFroms `<name>-conn`, enumerates `TOPIC_*` vars and POSTs each to the Lenses HQ REST API; **no topics declared ⇒ seeds the legacy health-domain set** (backward compatible). The old `09-stream-processing-setup.yaml` is now a documented health-domain example (header note); its SQL transforms are unchanged.
2. **`<name>-conn` normalization** — pure-CUE `conn-secret` output on `realtime-platform.cd.yaml` (mirrors postgresql/redis) with `KAFKA_BOOTSTRAP_SERVERS`, `KAFKA_SCHEMA_REGISTRY_URL`, `LENSES_URL`, `LENSES_HQ_URL`, `LENSES_WS_URL`, `REALTIME_PLATFORM_NAME`, `REALTIME_INTEGRATION_ENABLED`, `TOPIC_<name>`. **Additive** — the five legacy `<name>-*-secret` refs stay one release. The composition's secret-copier mirrors `<name>-conn` into the `<name>-realtime` ns so the topic Job can read it.
3. **realtime-service scaffold flavor** — NEW `realtime-service.cd.yaml` (knative-service shape, `requires-source-code: true`); emits an AppContainerClaim with `services[].flavor: realtime`. `ApplicationClaim` XRD gains optional `serviceFlavor: [webservice,realtime]` (default webservice); `AppContainerClaim` XRD `services[]` gains optional `flavor`; app-container composition passes `flavor → serviceFlavor`; application-claim-composition mscv Job gains `SERVICE_FLAVOR` env and, for `realtime`+python/fastapi, overwrites `src/main.py` with a `create_realtime_agent_app` variant and seds `aiokafka`+`websockets` into pyproject. No new template repo, no new framework enum.
4. **webservice realtime binding parity** — `webservice.cd.yaml` and `webservice-shape.yaml` both now envFrom `<realtime>-conn` (`optional:false`, day-0 lesson). webservice-shape also gains the `realtime?` param. Legacy 5 secrets untouched.
5. **services[] plumbing** — `submit_use_case._SCAFFOLD_TYPES = (webservice, realtime-service)`; `_needs_scaffold`/`_webservice_services` recognize `realtime-service` and tag `flavor: realtime`; `_validate_identity_topology` counts realtime-service as exposable. The dict flows verbatim through `k8s_claim_client` (no change there) into `services[]`.
6. **WS auth** — `create_realtime_agent_app` gains `verify_token` hook + a default env-based gate keyed off `JWT_ISSUER_URI` (from `<identity>-conn`); `/ws` + `/ws/events` close 4401 on failure (open when no identity bound — backward compatible). The realtime-service CD optionally renders Istio `RequestAuthentication`+`AuthorizationPolicy` when `jwtIssuer` is supplied literally.
7. OAM north-star example at `factory/production-lines/traditional-cloud/adapters/catalog/examples/realtime-streaming-example.yaml`.

### Deferred (with reason)
- **Istio RequestAuthentication from `<identity>-conn`** — Istio RA needs a literal `issuer`/`jwksUri` at render time; CUE cannot read them from the conn Secret. Implemented as opt-in `jwtIssuer`/`jwtJwksUri` CD params; the in-service `verify_token` gate is the always-on path. Full mesh-edge JWT keyed automatically off auth0 needs an external-secret→CR copier (out of RT-1 scope).
- **Full cryptographic JWT signature verification in `verify_token`** — default gate is presence/issuer-config only; services needing signature checks pass an explicit `verify_token`. (plan §8 limit.)
- **TOPICS comma-joined env / strings.ToUpper** — vela 1.10.6 CUE has no `strings` without imports; emit per-topic `TOPIC_<name>`/`CONSUME_<name>`/`PRODUCE_<name>` vars instead.
- **Zero-touch e2e smoke script** — example OAM shipped; the live pytest/bash assertion harness (Lenses topic GET, WS 401-without-JWT) needs a running cluster (no kubectl/az in this task).
- **per-topic partitions/retention in the Job** — declared topics use `TOPIC_PARTITIONS`/`TOPIC_RETENTION` job-level defaults (3 / 604800000); per-topic overrides would need array-JSON serialization which patch-and-transform lacks. XRD/CD still carry per-topic values for future use.

### Conflict-risk on submit_use_case.py
Changes are additive and in distinct regions from the graphql work: a new class constant `_SCAFFOLD_TYPES`, two `type != "webservice"` → `not in _SCAFFOLD_TYPES` swaps, one `entry["flavor"]` append, and one tuple-literal extension in `_validate_identity_topology`. No edits to `_derive_framework`, `_FW_FOR_LANG`, or any graphql/nodejs language mapping. `graphql-gateway.cd.yaml` and `factory/substrate/crossplane/graphql*` untouched.
