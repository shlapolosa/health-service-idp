...
FastAPI microservice implementing the Application Architect agent with Anthropic LLM integration.
"""
...
from .application_architect import ApplicationArchitectAgent, AnalysisResult
...
agent: Optional[ApplicationArchitectAgent] = None
...
async def lifespan(app: FastAPI):
    ...
    # Startup
    logger.info("Starting Application Architect Anthropic service...")
    
    try:
        agent = ApplicationArchitectAgent()
        await agent.initialize()
        logger.info("Application Architect agent initialized successfully")
    ...
    # Shutdown
    logger.info("Shutting down Application Architect Anthropic service...")
    if agent:
        await agent.cleanup()
...
app = FastAPI(
    title="Application Architect Anthropic Service",
    description="Microservice for application architect using deterministic",
    version="1.0.0",
    lifespan=lifespan
)


async def get_agent() -> ApplicationArchitectAgent:
...
async def health_check():
    ...
    return {"status": "healthy", "service": "application-architect-deterministic"}


@app.post("/analyze-requirements", response_model=AgentResponseModel)
async def analyze_requirements(
    request: AgentRequestModel,
    agent_instance: ApplicationArchitectAgent = Depends(get_agent)
) -> AgentResponseModel:
    """
    ...
    try:
        ...
        # Process with agent
        from .application_architect import AgentTask
        task = AgentTask(
