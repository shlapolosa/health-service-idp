...
FastAPI microservice implementing the Business Analyst agent with Anthropic LLM integration.
"""
...
from .business_analyst import BusinessAnalystAgent, AnalysisResult
...
async def lifespan(app: FastAPI):
    ...
    # Startup
    logger.info("Starting Business Analyst Anthropic service...")
    
    try:
        agent = BusinessAnalystAgent()
        await agent.initialize()
        logger.info("Business Analyst agent initialized successfully")
    ...
    # Shutdown
    logger.info("Shutting down Business Analyst Anthropic service...")
    if agent:
        await agent.cleanup()
...
app = FastAPI(
    title="Business Analyst Anthropic Service",
    description="Microservice for business analysis using Anthropic LLM",
    version="1.0.0",
    lifespan=lifespan
)


async def get_agent() -> BusinessAnalystAgent:
...
async def health_check():
    ...
    return {"status": "healthy", "service": "business-analyst-anthropic"}


@app.post("/analyze-requirements", response_model=AgentResponseModel)
async def analyze_requirements(
    request: AgentRequestModel,
    agent_instance: BusinessAnalystAgent = Depends(get_agent)
) -> AgentResponseModel:
    """
...
if __name__ == "__main__":
    ...
    uvicorn.run(app, host="0.0.0.0", port=port)[your existing code here]
