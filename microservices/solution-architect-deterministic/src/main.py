...
FastAPI microservice implementing the Solution Architect agent with Anthropic LLM integration.
"""
...
from .solution_architect import SolutionArchitectAgent, AnalysisResult
...
agent: Optional[SolutionArchitectAgent] = None
...
async def lifespan(app: FastAPI):
    ...
    # Startup
    logger.info("Starting Solution Architect Anthropic service...")
    ...
    # Shutdown
    logger.info("Shutting down Solution Architect Anthropic service...")
    if agent:
        await agent.cleanup()
...
app = FastAPI(
    title="Solution Architect Anthropic Service",
    description="Microservice for solution architect using deterministic",
    version="1.0.0",
    lifespan=lifespan
)


async def get_agent() -> SolutionArchitectAgent:
...
async def health_check():
    ...
    return {"status": "healthy", "service": "solution-architect-deterministic"}


@app.post("/analyze-requirements", response_model=AgentResponseModel)
async def analyze_requirements(
    request: AgentRequestModel,
    agent_instance: SolutionArchitectAgent = Depends(get_agent)
) -> AgentResponseModel:
    """
    ...
    try:
        ...
        # Process with agent
        from .solution_architect import AgentTask
        task = AgentTask(
