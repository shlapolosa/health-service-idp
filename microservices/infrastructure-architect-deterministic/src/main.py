...
FastAPI microservice implementing the Infrastructure Architect agent with Anthropic LLM integration.
"""
...
from .infrastructure_architect import InfrastructureArchitectAgent, AnalysisResult
...
agent: Optional[InfrastructureArchitectAgent] = None
...
async def lifespan(app: FastAPI):
    ...
    # Startup
    logger.info("Starting Infrastructure Architect Anthropic service...")
    
    try:
        agent = InfrastructureArchitectAgent()
        await agent.initialize()
        logger.info("Infrastructure Architect agent initialized successfully")
    ...
    # Shutdown
    logger.info("Shutting down Infrastructure Architect Anthropic service...")
    if agent:
        await agent.cleanup()
...
app = FastAPI(
    title="Infrastructure Architect Anthropic Service",
    description="Microservice for infrastructure architect using deterministic",
    version="1.0.0",
    lifespan=lifespan
)


async def get_agent() -> InfrastructureArchitectAgent:
...
async def health_check():
    ...
    return {"status": "healthy", "service": "infrastructure-architect-deterministic"}


@app.post("/analyze-requirements", response_model=AgentResponseModel)
async def analyze_requirements(
    request: AgentRequestModel,
    agent_instance: InfrastructureArchitectAgent = Depends(get_agent)
) -> AgentResponseModel:
    """
    ...
    try:
        ...
        # Process with agent
        from .infrastructure_architect import AgentTask
        task = AgentTask(
